...
function CartAddModal({ guitar, setModal, modal, setModalSuccess }: PropsType): JSX.Element {
  ...
  // const modalFocusTrap = createFocusTrap('#modal--add');
  ...
  const onSubmit = () => {
    const cartGuitarWithNewOne = cartGuitars.slice();
    cartGuitarWithNewOne.unshift(JSON.parse(JSON.stringify(guitar)));
    dispatch(loadCartGuitars(cartGuitarWithNewOne));
    setModal(false);
